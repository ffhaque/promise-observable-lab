...
export class ComparisonRunnerService {
  ...
  restartClock(): number { this.epoch = performance.now(); return this.epoch; }
  currentEpoch(): number { return this.epoch; }
  now(): number { return Math.max(0, performance.now() - this.epoch); }
  ...
  log(state: DemoState, type: DemoEventType, message: string, requestId?: number): void {
    const event: DemoEvent = { timestampMs: this.now(), type, message, ...(requestId === undefined ? {} : { requestId }) };
